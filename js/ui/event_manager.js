const eventManager = (() => {
    ...
    function handleBodyClick(event) {
        ...
        if (button?.closest('.chart-download-btn')) { exportTab.exportSingleChart(button); return; }
        if (button?.closest('.table-download-png-btn')) { exportTab.exportTablePNG(button); return; }
        ...
        if (button?.closest('#presentation-tab-pane') && button.id.startsWith('download-') && !button.disabled && !button.classList.contains('chart-download-btn') && !button.classList.contains('table-download-png-btn')) { presentationTab.handleDownloadClick(button.id); return; }
    ...
    function handleExportClick(button) {
        ...
        if (exportType.endsWith('-zip')) {
             ...
             exportTab.exportCategoryZip(category, app.getRawData(), bruteForceManager.getAllResults(), state.getCurrentCohort(), t2CriteriaManager.getAppliedCriteria(), t2CriteriaManager.getAppliedLogic());
        } else {
