const exportService = (() => {
    ...
    function generateMarkdownTableString(dataOrStats, tableType, kollektiv, criteria = null, logic = null, options = {}) {
        try {
            let headers = [], rows = [], title = ''; const kollektivDisplayName = getCohortDisplayName(kollektiv); const escMD = escapeHTML; const na = '--'; const formatCriteriaFunc = typeof studyT2CriteriaManager !== 'undefined' ? studyT2CriteriaManager.formatCriteriaForDisplay : (c, l) => 'N/A'; const t2CriteriaLabelShort = options.t2CriteriaLabelShort || 'T2';
    ...
    function generateComprehensiveReportHTML(data, bfResultsForCurrentKollektiv, kollektiv, criteria, logic) {
        try {
            const allBruteForceResults = bfResultsForCurrentKollektiv;
                 ...
                 if (statsDataForAllKollektive && state) {
                     PUBLICATION_CONFIG.sections.forEach(mainSection => {
                         mainSection.subSections.forEach(subSection => {
                            ...
                            const mdContent = publicationTab.getSectionContentForExport(subSection.id, state.getCurrentPublikationLang(), statsDataForAllKollektive, commonDataForPub);
                            ...
                            else if (subSection.id.startsWith('methoden')) { typeKey = 'PUBLICATION_METHODS_MD'; subSectionLabel = subSection.label.replace(/ /g, '_').replace(/[^a-zA-Z0-9_]/g, ''); }
                            else if (subSection.id.startsWith('ergebnisse')) { typeKey = 'PUBLICATION_RESULTS_MD'; subSectionLabel = subSection.label.replace(/ /g, '_').replace(/[^a-zA-Z0-9_]/g, ''); }
