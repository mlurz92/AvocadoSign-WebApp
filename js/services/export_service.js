const exportService = (() => {
    ...
    function downloadFile(content, filename, mimeType) {
        try {
            if (content === null || content === undefined) {
                 ...
                 uiManager.showToast(`Export fehlgeschlagen: Keine Daten für ${filename} generiert.`, 'warning');
                 return false;
            }
            let blob;
            if (content instanceof Blob) {
                ...
                if (blob.size === 0) {
                    ...
                    uiManager.showToast(`Export fehlgeschlagen: Leere Datei für ${filename} generiert.`, 'warning');
                    return false;
                }
            } else {
            ...
            uiManager.showToast(`Fehler beim Herunterladen der Datei '${filename}'.`, 'danger');
            return false;
        }
    }
    ...
    function generateBruteForceTXTString(resultsData) {
        ...
        try {
            ...
            const { results, metric, duration, totalTested, cohort, nTotal, nPlus, nMinus } = resultsData; // Changed kollektiv to cohort
            const cohortName = getCohortDisplayName(cohort); // Changed kollektivName to cohortName
            const bestResult = results[0];
            let report = `Brute-Force Optimierungsbericht\r\n==================================================\r\n`;
            report += `Datum der Analyse: ${new Date().toLocaleString('de-DE')}\r\n`;
            report += `Analysiertes Kollektiv: ${cohortName}\r\n`;
            report += `   - Gesamtzahl Patienten: ${formatNumber(nTotal, 0, 'N/A')}\r\n`; // Changed nGesamt to nTotal
    ...
    function generateMarkdownTableString(dataOrStats, tableType, kollektiv, criteria = null, logic = null, options = {}) {
        try {
            let headers = [], rows = [], title = ''; const kollektivDisplayName = getCohortDisplayName(kollektiv); const escMD = uiManager.escapeHTML; const na = '--'; const formatCriteriaFunc = typeof studyT2CriteriaManager !== 'undefined' ? studyT2CriteriaManager.formatCriteriaForDisplay : (c, l) => 'N/A'; const t2CriteriaLabelShort = options.t2CriteriaLabelShort || 'T2';
            if (tableType === 'daten') { title = 'Datenliste'; headers = ['Nr', 'Name', 'Vorname', 'Geschl.', 'Alter', 'Therapie', 'N', 'AS', 'T2', 'Bemerkung']; if(!Array.isArray(dataOrStats)) return `# ${title}...\n\nFehler: Ungültige Daten.`; rows = dataOrStats.map(p => [p.id, p.name || '', p.firstName || '', p.sex || '', p.age ?? '', getCohortDisplayName(p.therapy), p.nStatus ?? na, p.asStatus ?? na, p.t2Status ?? na, p.notes || ''].map(escMD)); } // Changed keys to English
            else if (tableType === 'auswertung') { title = 'Auswertungstabelle'; headers = ['Nr', 'Name', 'Therapie', 'N', 'AS', 'T2', 'N+/N ges', 'AS+/AS ges', 'T2+/T2 ges']; if(!Array.isArray(dataOrStats)) return `# ${title}...\n\nFehler: Ungültige Daten.`; rows = dataOrStats.map(p => [p.id, p.name || '', getCohortDisplayName(p.therapy), p.nStatus ?? na, p.asStatus ?? na, p.t2Status ?? na, `${formatNumber(p.countPathologyNodesPositive, 0, '-')} / ${formatNumber(p.countPathologyNodes, 0, '-')}`, `${formatNumber(p.countASNodesPositive, 0, '-')} / ${formatNumber(p.countASNodes, 0, '-')}`, `${formatNumber(p.countT2NodesPositive, 0, '-')} / ${formatNumber(p.countT2Nodes, 0, '-')}`].map(escMD)); } // Changed keys to English
            else if (tableType === 'deskriptiv') { title = 'Deskriptive Statistik'; const stats = dataOrStats; if (!stats || !stats.patientCount) return `# ${title} (Kollektiv: ${kollektivDisplayName})\n\nKeine Daten verfügbar.`; const total = stats.patientCount; headers = ['Metrik', 'Wert']; const fLKRowMD = (lk) => `${formatNumber(lk?.median, 1, na)} (${formatNumber(lk?.min, 0, na)}-${formatNumber(lk?.max, 0, na)}) \\[Mean: ${formatNumber(lk?.mean, 1, na)} ± ${formatNumber(lk?.sd, 1, na)}\\]`; rows = [ ['Anzahl Patienten', total], ['Median Alter (Min-Max) \\[Mean ± SD\\]', `${formatNumber(stats.age?.median, 1, na)} (${formatNumber(stats.age?.min, 0, na)} - ${formatNumber(stats.age?.max, 0, na)}) \\[${formatNumber(stats.age?.mean, 1, na)} ± ${formatNumber(stats.age?.sd, 1, na)}\\]`], ['Geschlecht (m/w) (n / %)', `${stats.sex?.m ?? 0} / ${stats.sex?.f ?? 0} (${formatPercent((stats.sex?.m ?? 0) / total, 1)} / ${formatPercent((stats.sex?.f ?? 0) / total, 1)})`], ['Therapie (direkt OP / nRCT) (n / %)', `${stats.therapy?.['direkt OP'] ?? 0} / ${stats.therapy?.nRCT ?? 0} (${formatPercent((stats.therapy?.['direkt OP'] ?? 0) / total, 1)} / ${formatPercent((stats.therapy?.nRCT ?? 0) / total, 1)})`], ['N Status (+ / -) (n / %)', `${stats.nStatus?.plus ?? 0} / ${stats.nStatus?.minus ?? 0} (${formatPercent((stats.nStatus?.plus ?? 0) / total, 1)} / ${formatPercent((stats.nStatus?.minus ?? 0) / total, 1)})`], ['AS Status (+ / -) (n / %)', `${stats.asStatus?.plus ?? 0} / ${stats.asStatus?.minus ?? 0} (${formatPercent((stats.asStatus?.plus ?? 0) / total, 1)} / ${formatPercent((stats.asStatus?.minus ?? 0) / total, 1)})`], ['T2 Status (+ / -) (n / %)', `${stats.t2Status?.plus ?? 0} / ${stats.t2Status?.minus ?? 0} (${formatPercent((stats.t2Status?.plus ?? 0) / total, 1)} / ${formatPercent((stats.t2Status?.minus ?? 0) / total, 1)})`], ['Median LK N ges. (Min-Max) \\[Mean ± SD\\]', fLKRowMD(stats.lnCounts?.n?.total)], ['Median LK N+ (Min-Max) \\[Mean ± SD\\] (nur N+ Pat.)', fLKRowMD(stats.lnCounts?.n?.plus)], ['Median LK AS ges. (Min-Max) \\[Mean ± SD\\]', fLKRowMD(stats.lnCounts?.as?.total)], ['Median LK AS+ (Min-Max) \\[Mean ± SD\\] (nur AS+ Pat.)', fLKRowMD(stats.lnCounts?.as?.plus)], ['Median LK T2 ges. (Min-Max) \\[Mean ± SD\\]', fLKRowMD(stats.lnCounts?.t2?.total)], ['Median LK T2+ (Min-Max) \\[Mean ± SD\\] (nur T2+ Pat.)', fLKRowMD(stats.lnCounts?.t2?.plus)] ].map(r => r.map(escMD)); }
            else if (tableType === 'praes_as_perf') { title = `Diagnostische Güte (AS) für Kollektive`; const { statsGesamt, statsDirektOP, statsNRCT } = dataOrStats || {}; if (!statsGesamt && !statsDirektOP && !statsNRCT) return `# ${title}\n\nFehler: Ungültige Daten.`; headers = ['Kollektiv', 'Sens. (95% CI)', 'Spez. (95% CI)', 'PPV (95% CI)', 'NPV (95% CI)', 'Acc. (95% CI)', 'AUC (95% CI)']; const fRow = (s, k) => { const d = getCohortDisplayName(k); if (!s || typeof s.matrix !== 'object') return [d + ' (N=?)', na, na, na, na, na, na].map(escMD); const n = s.matrix ? (s.matrix.tp + s.matrix.fp + s.matrix.fn + s.matrix.tn) : 0; const fCI_p = (m, ky) => { const dig = (ky === 'f1' || ky === 'auc') ? 3 : 1; const isP = !(ky === 'f1' || ky === 'auc'); return formatCI(m?.value, m?.ci?.lower, m?.ci?.upper, dig, isP, na); }; return [`${d} (N=${n})`, fCI_p(s.sens, 'sens'), fCI_p(s.spec, 'spec'), fCI_p(s.ppv, 'ppv'), fCI_p(s.npv, 'npv'), fCI_p(s.acc, 'acc'), fCI_p(s.auc, 'auc')].map(escMD); }; rows = [ fRow(statsGesamt, 'Gesamt'), fRow(statsDirektOP, 'direkt OP'), fRow(statsNRCT, 'nRCT') ]; }
            else if (tableType === 'praes_as_vs_t2_perf' || tableType === 'praes_as_vs_t2_comp') { const { statsAS, statsT2 } = dataOrStats || {}; title = `Vergleich Diagnostische Güte (AS vs. ${escMD(t2CriteriaLabelShort)})`; if (!statsAS || !statsT2) return `# ${title} (Kollektiv: ${kollektivDisplayName})\n\nFehler: Ungültige Daten für Vergleich.`; headers = ['Metrik', 'AS (Wert, 95% CI)', `${escMD(t2CriteriaLabelShort)} (Wert, 95% CI)`]; const fRow = (mKey, nm, isP = true, d = 1) => { const mAS = statsAS[mKey]; const mT2 = statsT2[mKey]; const dig = (mKey === 'auc' || mKey === 'f1') ? 3 : d; const vAS = formatCI(mAS?.value, mAS?.ci?.lower, mAS?.ci?.upper, dig, isP, na); const vT2 = formatCI(mT2?.value, mT2?.ci?.lower, mT2?.ci?.upper, dig, isP, na); return [nm, vAS, vT2]; }; rows = [ fRow('sens', 'Sensitivität'), fRow('spec', 'Spezifität'), fRow('ppv', 'PPV'), fRow('npv', 'NPV'), fRow('acc', 'Accuracy'), fRow('balAcc', 'Balanced Accuracy'), fRow('f1', 'F1-Score', false, 3), fRow('auc', 'AUC', false, 3) ].map(r => r.map(escMD)); }
    ...
    function generateFilteredDataCSVString(data) {
       ...
       try {
           const columns = ["id", "name", "firstName", "birthDate", "sex", "age", "therapy", "examDate", "nStatus", "countPathologyNodes", "countPathologyNodesPositive", "asStatus", "countASNodes", "countASNodesPositive", "t2Status", "countT2Nodes", "countT2NodesPositive", "notes"]; // Changed columns to English
    ...
    function _getPublicationSectionContentForExport(sectionId, lang, allKollektivStats, commonData) {
        try {
            ...
            // Fallback for publicationTextGenerator, e.g. if not loaded in worker context
            if (sectionId === 'abstract_main') {
                 return publicationTab._getAbstractText(commonData); // direkt aus publication_tab zugreifen
            }
    ...
    function generateComprehensiveReportHTML(data, bfResultsForCurrentKollektiv, kollektiv, criteria, logic) {
        try {
            ...
            html += `<div class="meta-info"><h3>Analysekonfiguration</h3><ul>`; if (config.INCLUDE_KOLLEKTIV_INFO) html += `<li><strong>Analysiertes Kollektiv:</strong> ${kollektivName} (N=${statsDataForCurrentKollektiv?.descriptive?.patientCount || 0})</li>`; if (config.INCLUDE_T2_CRITERIA) html += `<li><strong>Angewandte T2-Kriterien ('${appliedCriteriaDisplayName}'):</strong> Logik: ${logic}, Kriterien: ${criteriaString}</li>`; html += `</ul></div>`;
            if (config.INCLUDE_DESCRIPTIVES_TABLE && statsDataForCurrentKollektiv?.descriptive) { html += `<h2>Deskriptive Statistik</h2>`; html += `<table><thead><tr><th>Metrik</th><th>Wert (Median)</th><th>Mean</th><th>SD</th><th>Min</th><th>Max</th></tr></thead><tbody>`; const d = statsDataForCurrentKollektiv.descriptive; const na = '--'; const fv = (v, dig = 1, useStd = false) => formatNumber(v, dig, na, useStd); const fP = (v, dig = 1) => formatPercent(v, dig, na); const addRowHTML = (l, vl=na, m=na, s=na, mn=na, mx=na) => `<tr><td>${l}</td><td>${vl}</td><td>${m}</td><td>${s}</td><td>${mn}</td><td>${mx}</td></tr>`; html += addRowHTML('Alter (Jahre)', fv(d.age?.median, 1), fv(d.age?.mean, 1), fv(d.age?.sd, 1), fv(d.age?.min, 0), fv(d.age?.max, 0)); html += addRowHTML('Geschlecht Männlich (n / %)', `${d.sex?.m ?? 0} / ${fP(d.patientCount > 0 ? (d.sex?.m ?? 0) / d.patientCount : NaN, 1)}`); html += addRowHTML('Geschlecht Weiblich (n / %)', `${d.sex?.f ?? 0} / ${fP(d.patientCount > 0 ? (d.sex?.f ?? 0) / d.patientCount : NaN, 1)}`); html += addRowHTML('Therapie direkt OP (n / %)', `${d.therapy?.['direkt OP'] ?? 0} / ${fP(d.patientCount > 0 ? (d.therapy?.['direkt OP'] ?? 0) / d.patientCount : NaN, 1)}`); html += addRowHTML('Therapie nRCT (n / %)', `${d.therapy?.nRCT ?? 0} / ${fP(d.patientCount > 0 ? (d.therapy?.nRCT ?? 0) / d.patientCount : NaN, 1)}`); html += addRowHTML('N Status (+ / %)', `${d.nStatus?.plus ?? 0} / ${fP(d.patientCount > 0 ? (d.nStatus?.plus ?? 0) / d.patientCount : NaN, 1)}`); html += addRowHTML('AS Status (+ / %)', `${d.asStatus?.plus ?? 0} / ${fP(d.patientCount > 0 ? (d.asStatus?.plus ?? 0) / d.patientCount : NaN, 1)}`); html += addRowHTML('T2 Status (+ / %)', `${d.t2Status?.plus ?? 0} / ${fP(d.patientCount > 0 ? (d.t2Status?.plus ?? 0) / d.patientCount : NaN, 1)}`); const fLK = (lk) => `${fv(lk?.median,1)} (${fv(lk?.min,0)}-${fv(lk?.max,0)})`; html += addRowHTML('LK N gesamt (Median (Min-Max))', fLK(d.lnCounts?.n?.total), fv(d.lnCounts?.n?.total?.mean,1), fv(d.lnCounts?.n?.total?.sd,1),fv(d.lnCounts?.n?.total?.min,0), fv(d.lnCounts?.n?.total?.max,0)); html += addRowHTML('LK N+ (Median (Min-Max), nur N+ Pat.)', fLK(d.lnCounts?.n?.plus), fv(d.lnCounts?.n?.plus?.mean,1), fv(d.lnCounts?.n?.plus?.sd,1),fv(d.lnCounts?.n?.plus?.min,0), fv(d.lnCounts?.n?.plus?.max,0)); html += addRowHTML('LK AS gesamt (Median (Min-Max))', fLK(d.lnCounts?.as?.total), fv(d.lnCounts?.as?.total?.mean,1), fv(d.lnCounts?.as?.total?.sd,1),fv(d.lnCounts?.as?.total?.min,0), fv(d.lnCounts?.as?.total?.max,0)); html += addRowHTML('LK AS+ (Median (Min-Max), nur AS+ Pat.)', fLK(d.lnCounts?.as?.plus), fv(d.lnCounts?.as?.plus?.mean,1), fv(d.lnCounts?.as?.plus?.sd,1),fv(d.lnCounts?.as?.plus?.min,0), fv(d.lnCounts?.as?.plus?.max,0)); html += addRowHTML('LK T2 gesamt (Median (Min-Max))', fLK(d.lnCounts?.t2?.total), fv(d.lnCounts?.t2?.total?.mean,1), fv(d.lnCounts?.t2?.total?.sd,1),fv(d.lnCounts?.t2?.total?.min,0), fv(d.lnCounts?.t2?.total?.max,0)); html += addRowHTML('LK T2+ (Median (Min-Max), nur T2+ Pat.)', fLK(d.lnCounts?.t2?.plus), fv(d.lnCounts?.t2?.plus?.mean,1), fv(d.lnCounts?.t2?.plus?.sd,1),fv(d.lnCounts?.t2?.plus?.min,0), fv(d.lnCounts?.t2?.plus?.max,0)); html += `</tbody></table>`; }
    ...
    function exportStatistikCSV(data, kollektiv, criteria, logic) {
        ...
        try { stats = statisticsService.calculateAllPublicationStats(data, criteria, logic, allBruteForceResults)[kollektiv]; } catch(e) { uiManager.showToast("Fehler bei Statistikberechnung für CSV.", "danger"); return; }
        if (!stats) { uiManager.showToast("Keine Statistikdaten zum Exportieren für dieses Kollektiv.", "warning"); return; }
        try { csvString = generateStatistikCSVString(stats, kollektiv, criteria, logic); } catch(e) { uiManager.showToast("Fehler bei CSV-Erstellung.", "danger"); return; }
        if (csvString === null || csvString.length === 0) { uiManager.showToast("CSV-Generierung ergab leere Datei.", "warning"); return; }
        const filename = generateFilename('STATS_CSV', kollektiv, 'csv');
        if(downloadFile(csvString, filename, 'text/csv;charset=utf-8;')) uiManager.showToast(`Statistik als CSV exportiert: ${filename}`, 'success');
    }

    function exportBruteForceReport(resultsData) {
        if (!resultsData || !resultsData.results || resultsData.results.length === 0) {
             uiManager.showToast("Keine Brute-Force Ergebnisse zum Exportieren vorhanden.", "warning");
             return;
        }
        let txtString = null;
        try {
            ...
            uiManager.showToast("Fehler bei TXT-Erstellung.", "danger");
            return;
        }
        if (txtString === null || txtString.length === 0) {
            uiManager.showToast("TXT-Generierung ergab leere Datei.", "warning");
            return;
        }
        const filename = generateFilename('BRUTEFORCE_TXT', resultsData.cohort, 'txt'); // Changed resultsData.kollektiv to resultsData.cohort
        if(downloadFile(txtString, filename, 'text/plain;charset=utf-8;')) {
            uiManager.showToast(`Brute-Force Bericht exportiert: ${filename}`, 'success');
        }
    }
    ...
    function exportTableMarkdown(dataOrStats, tableType, kollektiv, criteria = null, logic = null, options = {}) {
        ...
        try { mdString = generateMarkdownTableString(dataOrStats, tableType, kollektiv, criteria, logic, options); } catch(e) { uiManager.showToast(`Fehler bei MD-Erstellung (${tableType}).`, "danger"); return; }
        if (mdString === null || mdString.length === 0) { uiManager.showToast(`MD-Generierung ergab leere Datei (${tableType}).`, "warning"); return; }
        ...
        if(downloadFile(mdString, filename, 'text/markdown;charset=utf-8;')) uiManager.showToast(`${title} als Markdown exportiert: ${filename}`, 'success');
    }

    function exportFilteredDataCSV(data, kollektiv) {
       let csvString = null;
       try { csvString = generateFilteredDataCSVString(data); } catch(e) { uiManager.showToast("Fehler bei Rohdaten-CSV-Erstellung.", "danger"); return; }
       if (csvString === null || csvString.length === 0) { uiManager.showToast("Rohdaten-CSV-Generierung ergab leere Datei.", "warning"); return; }
       const filename = generateFilename('FILTERED_DATA_CSV', kollektiv, 'csv');
       if(downloadFile(csvString, filename, 'text/csv;charset=utf-8;')) uiManager.showToast(`Gefilterte Daten als CSV exportiert: ${filename}`, 'success');
   }

    function exportComprehensiveReportHTML(data, bfResultsForCurrentKollektiv, kollektiv, criteria, logic) {
        let htmlString = null;
        try { htmlString = generateComprehensiveReportHTML(data, bfResultsForCurrentKollektiv, kollektiv, criteria, logic); } catch(e) { uiManager.showToast("Fehler bei HTML-Report-Erstellung.", "danger"); return; }
        if (htmlString === null || htmlString.length === 0) { uiManager.showToast("HTML-Report-Generierung ergab leere Datei.", "warning"); return; }
        const filename = generateFilename('COMPREHENSIVE_REPORT_HTML', kollektiv, 'html');
        if(downloadFile(htmlString, filename, 'text/html;charset=utf-8;')) uiManager.showToast(`Umfassender Bericht exportiert: ${filename}`, 'success');
    }

    async function exportSingleChart(chartElementId, format, kollektiv, options = {}) {
         const svgElement = document.getElementById(chartElementId)?.querySelector('svg'); if (!svgElement) { uiManager.showToast(`Diagramm '${chartElementId}' für Export nicht gefunden.`, 'danger'); return; }
         ...
         try {
             ...
             if (format === 'png') { uiManager.showToast(`Generiere PNG für Chart ${chartName}...`, 'info', 1500); blob = await convertSvgToPngBlob(svgElement); filenameKey = 'CHART_SINGLE_PNG'; mimeType = 'image/png'; ext = 'png'; }
             else if (format === 'svg') { uiManager.showToast(`Generiere SVG für Chart ${chartName}...`, 'info', 1500); blob = await convertSvgToSvgBlob(svgElement); filenameKey = 'CHART_SINGLE_SVG'; mimeType = 'image/svg+xml;charset=utf-8'; ext = 'svg'; }
             ...
             if (blob) {
                ...
                if (downloadFile(blob, filename, mimeType)) uiManager.showToast(`Chart ${chartName} als ${format.toUpperCase()} exportiert.`, 'success');
             } else {
                 throw new Error("Blob-Generierung fehlgeschlagen.");
             }
         } catch (error) { console.error(`Fehler beim Chart-Export (${chartName}, ${format}):`, error); uiManager.showToast(`Fehler beim Chart-Export (${format.toUpperCase()}).`, 'danger'); }
    }

     async function exportTablePNG(tableElementId, kollektiv, typeKey, tableName = 'Tabelle') {
         uiManager.showToast(`Generiere PNG für Tabelle ${tableName}...`, 'info', 1500);
         try {
             ...
             if (blob) {
                ...
                if(downloadFile(blob, filename, 'image/png')) uiManager.showToast(`Tabelle '${tableName}' als PNG exportiert.`, 'success');
             } else {
                throw new Error("Tabellen-Blob-Generierung fehlgeschlagen.");
             }
         } catch(error) { console.error(`Fehler beim Tabellen-PNG-Export für '${tableName}':`, error); uiManager.showToast(`Fehler beim Tabellen-PNG-Export für '${tableName}'.`, 'danger'); }
     }

    async function exportChartsZip(scopeSelector, zipTypeKey, kollektiv, format) {
         uiManager.showToast(`Starte ${format.toUpperCase()}-Export für sichtbare Charts & Tabellen...`, 'info', 2000);
         if (!window.JSZip) { uiManager.showToast("JSZip Bibliothek nicht geladen.", "danger"); return; }
         ...
         if (chartContainers.length === 0 && tableContainers.length === 0) { uiManager.showToast('Keine Diagramme oder Tabellen im aktuellen Sichtbereich gefunden.', 'warning'); return; }
         ...
         try {
             ...
             if (successCount > 0) {
                 ...
                 if (downloadFile(content, zipFilename, "application/zip")) uiManager.showToast(`${successCount} Objekt(e) erfolgreich als ${format.toUpperCase()} exportiert (ZIP).`, 'success');
             } else { uiManager.showToast(`Export (${format.toUpperCase()}) fehlgeschlagen: Keine Objekte konnten konvertiert werden.`, 'danger'); }
         } catch (error) { console.error(`Fehler beim Erstellen des ${format.toUpperCase()} ZIPs:`, error); uiManager.showToast(`Fehler beim Erstellen des ${format.toUpperCase()} ZIPs.`, 'danger'); }
     }

     async function exportCategoryZip(category, data, bfResults, kollektiv, criteria, logic) {
         uiManager.showToast(`Generiere ${category.toUpperCase()} ZIP-Paket...`, 'info', 2000);
          if (!window.JSZip) { uiManager.showToast("JSZip Bibliothek nicht geladen.", "danger"); return; }
         ...
         if(needsStats && data && data.length > 0 && criteria && logic) {
             ...
             } catch(e) { uiManager.showToast(`Fehler bei Statistikberechnung für ${category.toUpperCase()} ZIP.`, 'danger'); return; }
             if (!statsDataForAllKollektive || !statsDataForAllKollektive[kollektiv]) {
                 uiManager.showToast(`Statistikberechnung ergab keine Daten für ${category.toUpperCase()} ZIP für Kollektiv ${getCohortDisplayName(kollektiv)}.`, 'warning');
         ...
         try {
             if (['all-zip', 'csv-zip'].includes(category)) {
                 ...
                 if (data && data.length > 0) addFile(generateFilename('FILTERED_DATA_CSV', kollektiv, 'csv'), generateFilteredDataCSVString(dataProcessor.filterDataByCohort(data, kollektiv))); // Changed filterDataByKollektiv to filterDataByCohort
             }
             if (['all-zip', 'md-zip'].includes(category)) {
                 ...
                 if (data && data.length > 0) addFile(generateFilename('DATEN_MD', kollektiv, 'md'), generateMarkdownTableString(dataProcessor.filterDataByCohort(data, kollektiv), 'daten', kollektiv)); // Changed filterDataByKollektiv to filterDataByCohort
                 if (data && data.length > 0) addFile(generateFilename('AUSWERTUNG_MD', kollektiv, 'md'), generateMarkdownTableString(dataProcessor.filterDataByCohort(data, kollektiv), 'auswertung', kollektiv, criteria, logic)); // Changed filterDataByKollektiv to filterDataByCohort
                     ...
                     PUBLICATION_CONFIG.sections.forEach(mainSection => {
                         mainSection.subSections.forEach(subSection => {
                            const commonDataForPub = {
                                appName: APP_CONFIG.APP_NAME, appVersion: APP_CONFIG.APP_VERSION, currentKollektivName: getCohortDisplayName(kollektiv),
                                nOverall: statsDataForAllKollektive.Gesamt?.descriptive?.patientCount || 0, // Changed nGesamt to nOverall
                                nUpfrontSurgery: statsDataForAllKollektive['direkt OP']?.descriptive?.patientCount || 0,
                                nNRCT: statsDataForAllKollektive.nRCT?.descriptive?.patientCount || 0,
                                significanceLevel: APP_CONFIG.STATISTICAL_CONSTANTS.SIGNIFICANCE_LEVEL,
                                references: APP_CONFIG.REFERENCES_FOR_PUBLICATION || {},
                                bruteForceMetricForPublication: state.getPublicationBruteForceMetric() || PUBLICATION_CONFIG.defaultBruteForceMetricForPublication // Changed getCurrentPublikationBruteForceMetric to getPublicationBruteForceMetric
                            };
                            const mdContent = _getPublicationSectionContentForExport(subSection.id, state.getCurrentPublikationLang(), statsDataForAllKollektive, commonDataForPub); // Assuming getCurrentPublikationLang will be added to state.js or a global var
            ...
            if (filesAdded > 0) {
                ...
                if (downloadFile(content, zipFilename, "application/zip")) uiManager.showToast(`${filesAdded} Datei(en) erfolgreich im ${category.toUpperCase()} ZIP-Paket exportiert.`, 'success');
            } else { uiManager.showToast(`Keine Dateien für das ${category.toUpperCase()} ZIP-Paket gefunden oder generiert.`, 'warning'); }
         } catch (error) { console.error(`Fehler beim Erstellen des ${category.toUpperCase()} ZIP-Pakets:`, error); uiManager.showToast(`Fehler beim Erstellen des ${category.toUpperCase()} ZIP-Pakets.`, 'danger'); }
     }
     ...
     function exportPraesentationData(actionId, presentationData, kollektiv) {
            ...
            if (!presentationData) { uiManager.showToast("Keine Daten für Präsentationsexport verfügbar.", "warning"); return; }
            const { statsAS, statsT2, comparison, comparisonCriteriaSet, cohortForComparison, patientCountForComparison, t2ShortName } = presentationData || {};
            ...
            try {
                if (isAsPurView && actionId === 'download-performance-as-pur-csv') {
                     ...
                     const rows = Object.entries(allStatsData).map(([key, stats]) => { let k = key.replace('stats',''); let dN = (k === 'DirektOP') ? 'direkt OP' : (k === 'NRCT') ? 'nRCT' : k; if (!stats || typeof stats.matrix !== 'object') return [getCohortDisplayName(dN), 0, ...Array(21).fill(na), na]; const n = stats.matrix ? (stats.matrix.tp + stats.matrix.fp + stats.matrix.fn + stats.matrix.rn) : 0; const fRowData = (m, metric_k) => { const dig = (metric_k === 'f1' || metric_k === 'auc') ? 3 : 1; return [fVal(m?.value, dig, true), fVal(m?.ci?.lower, dig, true), fVal(m?.ci?.upper, dig, true)]; }; return [ getCohortDisplayName(dN), n, ...fRowData(stats.sens, 'sens'), ...fRowData(stats.spec, 'spec'), ...fRowData(stats.ppv, 'ppv'), ...fRowData(stats.npv, 'npv'), ...fRowData(stats.acc, 'acc'), ...fRowData(stats.balAcc, 'balAcc'), ...fRowData(stats.f1, 'f1'), ...fRowData(stats.auc, 'auc'), stats.sens?.method || na ]; });
                ...
                } else if (isAsVsT2View && actionId === 'download-performance-as-vs-t2-csv') { if (!statsAS || !statsT2) { uiManager.showToast("Vergleichsdaten für CSV fehlen.", "warning"); return; } const headers = ['Metrik', 'AS (Wert)', 'AS (95% CI)', 'T2 (Wert)', 'T2 (95% CI)', 'CI Methode AS', 'CI Methode T2']; const fRow = (mKey, nm, isP = true, d = 1) => { const mAS = statsAS[mKey]; const mT2 = statsT2[mKey]; const dig = (mKey === 'auc' || mKey === 'f1') ? 3 : d; const ciAS = `(${formatNumber(mAS?.ci?.lower, dig, na, true)} - ${formatNumber(mAS?.ci?.upper, dig, na, true)})`; const ciT2 = `(${formatNumber(mT2?.ci?.lower, dig, na, true)} - ${formatNumber(mT2?.ci?.upper, dig, na, true)})`; const valAS = formatNumber(mAS?.value, dig, na, true); const valT2 = formatNumber(mT2?.value, dig, na, true); return [nm, valAS, ciAS, valT2, ciT2, mAS?.method || na, mT2?.method || na]; }; const rows = [ fRow('sens', 'Sensitivität'), fRow('spec', 'Spezifität'), fRow('ppv', 'PPV'), fRow('npv', 'NPV'), fRow('acc', 'Accuracy'), fRow('balAcc', 'Balanced Accuracy'), fRow('f1', 'F1-Score', false, 3), fRow('auc', 'AUC', false, 3) ]; content = Papa.unparse([headers, ...rows], { delimiter: APP_CONFIG.EXPORT_SETTINGS.CSV_DELIMITER || ";" }); filenameKey = 'PRAES_AS_VS_T2_PERF_CSV'; extension = 'csv'; mimeType = 'text/csv;charset=utf-8;';
                ...
                } else if (actionId.includes('-chart-') || actionId.startsWith('dl-praes-')) { uiManager.showToast("Chart-Export wird über die Buttons am Chart selbst ausgelöst.", "info"); return; }
            } catch(error) {
                console.error(`Fehler bei Präsentationsexport ${actionId}:`, error);
                uiManager.showToast(`Fehler bei Präsentationsexport (${actionId}).`, "danger");
                return;
            }

            if(content !== null && filenameKey && extension && mimeType) { const filename = generateFilename(filenameKey, kollektiv, extension, options); success = downloadFile(content, filename, mimeType); if(success) uiManager.showToast(`Präsentationsdaten (${extension}) exportiert: ${filename}`, 'success'); }
            else if(!actionId.includes('-chart-') && !actionId.startsWith('dl-') ) { uiManager.showToast("Export für diese Option nicht verfügbar oder Daten fehlen/Fehler bei Generierung.", "warning"); }
      }
